...
<script>
  ...
  $(function() {
    ...
    if (resturant == '') {
    //  resturant_id =  $('#vendor_restaurant_select').find(":selected").val();
      $("#vendor_restaurant_select").change(function () {
        resturant_id =$(this).val();
      });
      //resturant_id = $("#vendor_restaurant_select option:selected").val();
    }else{
    ...
    $(".save_coupon_btn").click(function(){
      ...
      if(code == ''){
       ...
       //   console.log('done'+resturant_id+' '+expiresAt);
           ...
           database.collection('coupons').doc(id).set({'code':code,'description':description,'discount':discount,'expiresAt':expiresAt,'isEnabled':isEnabled,'id':id,'discountType':discountType,'image':photo,'resturant_id':vandorId}).then(function(result) {
