                        ...
                        <div class="form-group text-center m-t-20">
                            <div class="col-xs-12">
                                ...
                                <button type="button" onclick="loginBackClick()"
                                        class="btn btn-dark btn-lg btn-block text-uppercase waves-effect waves-light btn btn-primary">{{ trans('lang.log_in') }} {{ trans('lang.with_email') }}</button>
...
<script type="text/javascript">
    ...
    function loginClick() {
        ...
        var useruploadStory = database.collection('settings').doc("restaurant");
        useruploadStory.get().then(async function (snapshots) {
            var storydata = snapshots.data();
            if (storydata.auto_approve_restaurant == true) {
                firebase.auth().signInWithEmailAndPassword(email, password).then(function (result) {
                    ...
                    jQuery("#password_required").html(error.message);
                });
            } else if (storydata.auto_approve_restaurant == false) {
                firebase.auth().signInWithEmailAndPassword(email, password).then(function (result) {
                    var userEmail = result.user.email;
